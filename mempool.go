...
func (c *Client) GetMempoolInfo() (info *MempoolInfo, err error) {
	...
	if resp, err = c.Request(fmt.Sprintf("%s%s/mempool/info", apiEndpoint, c.Parameters.Network), http.MethodGet, nil); err != nil {
		return
	}
...
func (c *Client) GetMempoolTransactions() (transactions []string, err error) {
	...
	if resp, err = c.Request(fmt.Sprintf("%s%s/mempool/raw", apiEndpoint, c.Parameters.Network), http.MethodGet, nil); err != nil {
		return
	}
