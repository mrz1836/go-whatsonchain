...
func (c *Client) GetScriptHistory(scriptHash string) (history ScriptList, err error) {
	...
	if resp, err = c.Request(fmt.Sprintf("%s%s/script/%s/history", apiEndpoint, c.Parameters.Network, scriptHash), http.MethodGet, nil); err != nil {
		return
	}
...
func (c *Client) GetScriptUnspentTransactions(scriptHash string) (scriptList ScriptList, err error) {
	...
	if resp, err = c.Request(fmt.Sprintf("%s%s/script/%s/unspent", apiEndpoint, c.Parameters.Network, scriptHash), http.MethodGet, nil); err != nil {
		return
	}
